...
internal static class ExportAnnotations
{
    ...
    internal static void AppendGrainDxf(StringBuilder sb, PieceDefinition p, double dx, double dy)
    {
        if (p.Grain is not { Count: >= 2 }) return;
        var nl = "\r\n";
        var g0 = p.Grain[0];
    ...
    internal static void AppendNotchesDxf(StringBuilder sb, PieceDefinition p, double dx, double dy)
    {
        if (p.Notches is not { Count: > 0 }) return;
        var poly = p.Points;
        if (poly.Count < 3) return;
