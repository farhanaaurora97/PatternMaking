...
namespace PatternPro.Web.Controllers;
    ...
    IProductionCertificationService productionCertification) : Controller
{
    ...
    public IActionResult ApproveForCutting([FromBody] ProductionActionRequest req)
    {
        ...
        if (qcErrors.Count > 0)
            return BadRequest(new { message = "QC must pass before approval.", issues = qcErrors });
        ...
        string purpose = "factory")
    {
        ...
        try
        {
            var (bytes, contentType, fileName) = exportService.BuildExportPackage(
            ...
            return File(new MemoryStream(bytes, writable: false), contentType, fileName);
        }
